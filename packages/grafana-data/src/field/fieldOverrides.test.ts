  ...
  describe('given a frame field', () => {
    ...
    it('will apply targeted field overrides to the fields within nested frames', () => {
      ...
      const withOverrides = applyFieldOverrides({
        ...
        fieldConfigRegistry: customFieldRegistry,
        featureToggles: {
          nestedFramesFieldOverrides: true,
        },
      });
    ...
    it('will not apply targeted field overrides to the fields within nested frames if the feature toggle is not enabled', () => {
      const f0 = createDataFrame({
        name: 'A',
        fields: [
          {
      ...
      expect(withOverrides[0].fields[1].values[0][0].fields[1].config.max).toBeUndefined();
    });
  });
