...
func NewAlertmanager(
	...
	err = func() error {
		...
		defaultCfg, err := am.buildConfiguration(ctx, parsed) // TODO: This will need to be addressed when extracting config preparation
		if err != nil {
			return fmt.Errorf("unable to build default configuration: %w", err)
		}
...
func (am *Alertmanager) buildConfiguration(ctx context.Context, c *apimodels.PostableUserConfig) (remoteClient.UserGrafanaConfig, error) {
	...
	amConfig := c.AlertmanagerConfig        // Shallow copy.
	amConfig.Receivers = decryptedReceivers // Modify the shallow copy's receivers.
