...
func NewAlertmanager(
	...
	am := &Alertmanager{
		...
		// This hash is based on the unprepared config from the DB and intentionally doesn't include autogenerated routes.
		// If autogenerated routes and were included, then this hash could regularly change as rules are updated.
		defaultConfigHash: md5.Sum([]byte(cfg.DefaultConfig)),
		log:               logger,
		metrics:           metrics,
		mimirClient:       mc,
		orgID:             cfg.OrgID,
...
func (am *Alertmanager) CompareAndSendConfiguration(ctx context.Context, config alertingNotify.NotificationsConfiguration) (bool, error) {
...
func (am *Alertmanager) buildConfiguration(ctx context.Context, c alertingNotify.NotificationsConfiguration) (remoteClient.UserGrafanaConfig, error) {
	...
	decryptedReceivers, err := notifier.DecryptedGrafanaReceivers(APIReceiversToPostableAPIReceivers(c.Receivers), notifier.DecryptIntegrationSettings(ctx, am.crypto))
	if err != nil {
		return remoteClient.UserGrafanaConfig{}, fmt.Errorf("unable to decrypt receivers: %w", err)
	}
	amConfig := definition.PostableApiAlertingConfig{
		Config: definition.Config{
			Global:            nil, // Grafana does not have global.
			Route:             definition.AsGrafanaRoute(c.RoutingTree),
