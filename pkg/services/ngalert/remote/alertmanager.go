...
func NewAlertmanager(
	...
	err = func() error {
		...
		if err := notifier.AddAutogenConfig(ctx, am.log, nil, am.orgID, &parsed.AlertmanagerConfig, notifier.LogInvalidReceivers, am.features); err != nil {
			return fmt.Errorf("unable to add autogenerated routes: %w", err)
		}

		config, err := notifier.PostableAPIConfigToNotificationsConfiguration(parsed, alertingNotify.DynamicLimits{})
		if err != nil {
			return err
		}

		am.defaultConfigHash = config.Hash
		return nil
	}()
	if err != nil {
...
func (am *Alertmanager) buildConfiguration(ctx context.Context, c alertingNotify.NotificationsConfiguration) (remoteClient.UserGrafanaConfig, error) {
	...
	if err := amConfig.Validate(); err != nil {
		return remoteClient.UserGrafanaConfig{}, fmt.Errorf("invalid configuration: %w", err)
	}
