...
func WithNotificationSettings(notificationSettings map[int64]map[models.AlertRuleKey]models.ContactPointRouting) TestMultiOrgAlertmanagerOption {
	return func(opts *TestMultiOrgAlertmanagerOptions) {
		opts.notificationSettings = notificationSettings
	}
}
...
func NewTestMultiOrgAlertmanager(t *testing.T, opts ...TestMultiOrgAlertmanagerOption) *MultiOrgAlertmanager {
	...
	cfgStore.notificationSettings = options.notificationSettings
...
// WithMOA is a helper to validate alertmanagers when used via multiorgalertmanager.
// Since autogenerated routes and ExtraConfig preparation moved to multiorgalertmanager, this compat layer helps ensure
// the previous functionality remains intact.
func WithMOA(t *testing.T, cfg *definitions.PostableUserConfig, callback func(prepared alertingNotify.NotificationsConfiguration) (bool, error), opts ...TestMultiOrgAlertmanagerOption) (bool, error) {
	t.Helper()
	moa := NewTestMultiOrgAlertmanager(t, opts...)

	dbConfig, err := json.Marshal(cfg)
	require.NoError(t, err)

	prepared, err := moa.prepareApplyConfig(context.Background(), 1, &models.AlertConfiguration{AlertmanagerConfiguration: string(dbConfig)}, ErrorOnInvalidReceivers)
	if err != nil {
		return false, err
	}
