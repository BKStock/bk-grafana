...
func (moa *MultiOrgAlertmanager) SaveAndApplyDefaultConfig(ctx context.Context, orgId int64) error {
	...
	err = moa.configStore.SaveAlertmanagerConfigurationWithCallback(ctx, cmd, func(alertConfig models.AlertConfiguration) error {
		preparedCfg, err := moa.prepareApplyConfig(ctx, orgId, &alertConfig, LogInvalidReceivers)
		if err != nil {
			return err
		}
...
func (moa *MultiOrgAlertmanager) saveAndApplyConfig(ctx context.Context, org int64, cfg *definitions.PostableUserConfig) error {
	...
	return moa.configStore.SaveAlertmanagerConfigurationWithCallback(ctx, cmd, func(alertConfig models.AlertConfiguration) error {
		preparedCfg, err := moa.prepareApplyConfig(ctx, org, &alertConfig, ErrorOnInvalidReceivers) // Rollback save if apply fails.
		if err != nil {
			return err
		}
		_, err = am.ApplyConfig(ctx, preparedCfg)
