...
func (moa *MultiOrgAlertmanager) prepareApplyConfig(ctx context.Context, orgID int64, cfg *models.AlertConfiguration, onInvalid InvalidReceiversAction) (alertingNotify.NotificationsConfiguration, error) {
...
func (moa *MultiOrgAlertmanager) SaveAndApplyDefaultConfig(ctx context.Context, orgId int64) error {
	...
	err = moa.saveAndApplyDefaultConfig(ctx, orgAM, orgId)
	if err != nil {
		return err
	}
...
func (moa *MultiOrgAlertmanager) ApplyConfig(ctx context.Context, orgId int64, dbConfig *models.AlertConfiguration) error {
	...
	err = moa.applyConfig(ctx, am, dbConfig)
	if err != nil {
...
func (moa *MultiOrgAlertmanager) saveAndApplyConfig(ctx context.Context, orgID int64, cfg *definitions.PostableUserConfig) error {
	moa.alertmanagersMtx.RLock()
	defer moa.alertmanagersMtx.RUnlock()
	...
	return moa.saveAndApplyCmd(ctx, am, newSaveAMConfigCmd(string(cfgToSave), orgID, false))
}
...
func (moa *MultiOrgAlertmanager) saveAndApplyDefaultConfig(ctx context.Context, am Alertmanager, org int64) error {
	return moa.saveAndApplyCmd(ctx, am, newSaveAMConfigCmd(moa.settings.UnifiedAlerting.DefaultConfiguration, org, true))
}

// saveAndApplyCmd is a helper method for saving and applying a configuration.
// Caller should lock the alertmanagersMtx.
func (moa *MultiOrgAlertmanager) saveAndApplyCmd(ctx context.Context, am Alertmanager, cmd *models.SaveAlertmanagerConfigurationCmd) error {
	return moa.configStore.SaveAlertmanagerConfigurationWithCallback(ctx, cmd, func(alertConfig models.AlertConfiguration) error {
		preparedCfg, err := moa.prepareApplyConfig(ctx, alertConfig.OrgID, &alertConfig, ErrorOnInvalidReceivers) // Rollback save if apply fails.
		if err != nil {
			return err
		}
...
func (moa *MultiOrgAlertmanager) applyConfig(ctx context.Context, am Alertmanager, dbConfig *models.AlertConfiguration) error {
	preparedCfg, err := moa.prepareApplyConfig(ctx, dbConfig.OrgID, dbConfig, LogInvalidReceivers)
	if err != nil {
		return err
	}
	...
	if configChanged {
		if err = moa.configStore.MarkConfigurationAsApplied(ctx, &models.MarkConfigurationAsAppliedCmd{
			...
			moa.logger.Error("Failed to mark Alertmanager configuration as applied", "org", dbConfig.OrgID, "hash", dbConfig.ConfigurationHash, "error", err)
		}
	}
	return nil
}
